...
def do_coco_style_eval(gt_annos, dt_annos, current_classes, overlap_ranges):
    ...
    for i in range(overlap_ranges.shape[1]):
        for j in range(overlap_ranges.shape[2]):
            start, end, num_points = overlap_ranges[:, i, j]
            # Convert num_points to integer
            num_points = int(num_points)
            min_overlaps[:, i, j] = np.linspace(start, end, num_points)
            # min_overlaps[:, i, j] = np.linspace(*overlap_ranges[:, i, j])
    # mAP_bbox, mAP_bev, mAP_3d, mAP_aos, _, _, \
    #     _, _ = do_eval(gt_annos, dt_annos,
    #                    current_classes, min_overlaps,
    #                    compute_aos)
    _, _, _, _, mAP_bbox, mAP_bev, \
        mAP_3d, mAP_aos = do_eval(gt_annos, dt_annos,
                       current_classes, min_overlaps)
...
def kitti_eval(gt_annos,
    ...
    #                         [0.5, 0.25, 0.25, 0.5, 0.25]])
    overlap_0_5 = np.array([[0, 0.7, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5],
                            [0, 0.5, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25],
                            [0, 0.5, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25]])
